...
def create_request(request, role, ip, worker_num=None):
    ...
    req.addService(pg.Execute(
        'bash',
        "sudo /usr/local/etc/emulab/mkextrafs.pl /mnt; sudo chmod 777 -R /local /mnt; rsync -av /local/ /mnt/local/; sudo mount -o bind /mnt/local /local; sudo bash /local/repository/bootstrap.sh '{}' '{}' '{}' 2>&1 | sudo tee -a /logs/setup.log".format(
